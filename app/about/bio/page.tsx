...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!row) {
    return {
      title: "Bio",
      alternates: { canonical: "/about/bio" },
      openGraph: { url: absoluteUrl("/about/bio"), title: "Bio | Ross Belot" },
    };
  }
    ...
    return (
      <main id="main" className="essay-layout">
        <div className="essay-toolbar">
          <Link href="/about" className="essay-back-link">
            About
          </Link>
          <div className="essay-pdf-slot" aria-hidden="true" />
        </div>
        <header className="essay-header">
          <p className="essay-kicker">Bio</p>
          <h1 className="essay-title">Bio</h1>
        </header>
